...
```gradle
implementation "cn.lalaki:android_smtp_client:3.1.1" // Android

implementation "cn.lalaki:java_smtp_client:3.1.1" // Java
```

## API

[View code](https://github.com/lalakii/android_smtp_client/blob/master/app/src/main/java/cn/lalaki/mail/ui/MainActivity.java#L119)
```java
// The code must be executed in a thread.
new Thread(() -> {
    SmtpClient client = new SmtpClient.Builder().setLogEnabled(true).config(host, port, tlsMode, certVerifyDisable, username, password).build();
