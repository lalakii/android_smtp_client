...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, RadioGroup.OnCheckedChangeListener {
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.add_attachment) {
                    ...
                    .forResult(new OnResultCallbackListener() {
                        ...
                        public void onResult(List<FileBean> result) {
                            ...
                            if (attachmentCount != null) {
                                attachmentCount.setText(String.format("已选择附件数量: %s个", result.size()));
                            }
                        }
            ...
            new Thread(() -> {
                SmtpClient client = new SmtpClient.Builder().setLogEnabled(true).config(host, mPort, tlsMode, certVerifyDisable, username, password).build();
                client.sendMail(new SmtpClient.EmailMessage(to, subject, body, SmtpClient.ContentType.TEXT_PLAIN, attachments, SmtpClient.Priority.Normal), new SmtpClient.IMailSentListener() {
                    @Override
